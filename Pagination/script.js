...
function myFunction() {
  ...
  else
  {
	...
	if (y.matches) {
		if($(".fa").hasClass('up')){
			document.getElementById("rightContainer").style.marginTop="140px";
		}else{
...
async function clicked(i) 
{
	...
	{	
		...
		if (page.present)
		{
			...
			if (meth == 2) instructions.innerHTML="Mettre ? jour les attributs de Page "+page.id+" et les valeurs des compteurs";
			else instructions.innerHTML="Mettre ? jour les attributs de Page "+page.id+" ...";
		...
		document.getElementById("memory").style.border="solid #2C3E50 4px";
		document.getElementById("blockLabelArray"+(i+1)+"3").style.backgroundColor="#2B2E4A";
		instructions.innerHTML="V?rifier s'il y a de la place en MC ...";
		await sleep(2000);
		if (nombreDePageDansMC<capacity+1) 
		{
			...
			await sleep(3000);
			document.getElementById("memory").style.border="solid #2C3E50 4px";
			if (meth == 2) {instructions.innerHTML="Ins?rer Page "+page.id+" dans MC et mettre ? jour les compteurs ...";await sleep(3500);}
			else {instructions.innerHTML="Ins?rer Page "+page.id+" dans MC";await sleep(2000);}
			listePagesSelected.push(page1);
			await sleep(1000);
			instructions.innerHTML="Fin";
			await sleep(800);
		...
		await sleep(3000);
		document.getElementById("memory").style.border="solid #2C3E50 4px";
		instructions.innerHTML="Trouver un rempla?ant selon la m?thode "+MDR[meth-1]+" ...";
		await sleep(3000);
						...
						while (!trouv)
						{
							...
							while ((!trouv) && (i1<listePagesSelected1.length))
							{
								if ((listePagesSelected1[i1].present==1)&&(listePagesSelected1[i1].modif==M)&&(listePagesSelected1[i1].reference==R)) 
								{
									...
									instructions.innerHTML="C'est la bonne page ? etre remplacer";
		        					await sleep(5000);
								}	
								else 
								{
									if (listePagesSelected1[i1].present==1)
									{
										...
										instructions.innerHTML="Page "+listePagesSelected1[i1].id+" ne v?rifie pas les conditions recherch?s";
			        					await sleep(5000);
		...
		//**************** FIN AGING **************//	
		        ...
		        if (i != -1)
		        {
					...
					if (pageARemplacer!=null)
					{
						...
						if (meth == 2) 
						{
							instructions.innerHTML="Mettre ? jour les compteurs ...";
							await sleep(3000);
						}
						$("#arrayContainer").hide();
						instructions.innerHTML="La page ? remplacer: Page "+pageARemplacer.id+" du processus: "+listeProcessus[i].id;
						...
						await sleep(5000);
						instructions.innerHTML="V?rifier le champ modif de la page ? remplacer";
						if (pageARemplacer.modif == 1)
						{
							document.getElementById("blockLabelArray"+(recherchIndexPage(listeProcessus[i].listePages,pageARemplacer)+1)+"4").style.backgroundColor="rgb(72, 255, 0)";
							instructions.innerHTML="Page "+pageARemplacer.id+" a ?t? modifi?";
							await sleep(3000);
							instructions.innerHTML="R??crire Page "+pageARemplacer.id+" dans MS avant de la remplacer";
							await sleep(4000);
						} 			
						else 
						{
							document.getElementById("blockLabelArray"+(recherchIndexPage(listeProcessus[i].listePages,pageARemplacer)+1)+"4").style.backgroundColor="rgb(72, 255, 0)";
							instructions.innerHTML="Page "+pageARemplacer.id+" n'a pas ?t? modifi? => remplacement direct";
							await sleep(3000);	
						}
					};
					/////////////////////////////////////////
...
var sbmt=function() {
	...
	{	
		if (nbPiste!=3)
		{
			...
			else alert("Erreur dans le nombre de pages");
		}
  		else alert("La m?moire secondaire est pleine");
  	}
  	else 
  	{
  		alert("Veuiller choisir une m?thode de remplacement");
  	}
	inPageNumber.value = "";
	inProcessId.value = "";
};
...
var clock = setInterval(function() 
{
	...
	heap.repaint();
	defautPage.innerHTML="D?faut de page: "+page_faults;	
});
