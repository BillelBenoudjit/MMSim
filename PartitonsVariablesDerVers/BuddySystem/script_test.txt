...
function Heap() {
	...
	this.requestAllocation = function(process) {
		block = this.head;
			...
			block = block.next;
			if (block== null) {return false}; // Means we couldn't even find an initial valid block
		};

	    	while (((block.size/2) >= process.size) && (block.available)) {
			block.setSize(block.size/2);
			newBlock = new MemControlBlock(block.size);
			...
			block.next=newBlock;
			newBlock.prev=block; // Means we couldn't even find an initial valid block
		};
